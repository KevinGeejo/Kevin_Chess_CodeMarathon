using ChessCodeMarathon.Models;
using Npgsql;
using System.Data;
using NpgsqlTypes;
...
namespace ChessCodeMarathon.DAO
{
    public class PlayerDaoImplementation : IPlayerDao
    {
        ...
        public async Task<List<PlayerPerformanceView>> GetPlayersPerformance()
        {
            ...
            string query = @"SELECT 
                            ...
                            ROUND(
                                100.0 * SUM(CASE WHEN m.winner_id = p.player_id THEN 1 ELSE 0 END) / COUNT(m.match_id), 
                                2
                            ) AS win_percentage
                               FROM 
                             chess.Players p
                            LEFT JOIN 
                            chess.Matches m ON p.player_id IN (m.player1_id, m.player2_id)
                            GROUP BY 
            ...
            try
            {
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        p.FullName = (string)reader[0];
                        p.TotalMatchesPlayed = reader.GetInt32(1);
                        p.TotalWon = (Int64)reader[2];
                        p.WinPercentage = (decimal)reader[3];
                        plist.Add(p);
                    }
                }
                reader.Close();
